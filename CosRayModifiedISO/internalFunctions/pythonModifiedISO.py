...
def getModifiedISO_GCR_Flux_Default_Energies(W, Z):
    ...
    outputFluxArray = np.array([[energy, getModifiedISO_GCR_Flux_Single(
        W, Z, energy)] for energy in middleEnergies])

    return outputFluxArray


def getModifiedISO_GCR_Flux_Custom_Energies(W, Z, listOfEnergyBins):
    middleEnergies = (listOfEnergyBins[1:] + listOfEnergyBins[:-1])/2

    outputFluxArray = np.array([middleEnergies, getModifiedISO_GCR_Flux_Energy_Arr(
        W, Z, middleEnergies)])
    return outputFluxArray


def getModifiedISO_GCR_Flux_Time_Series_Custom_Energies(W_array, Z, listOfEnergyBins):
    middleEnergies = (listOfEnergyBins[1:] + listOfEnergyBins[:-1])/2

    flux = getModifiedISO_GCR_Flux_Energy_Arr_Time_Series(
        W_array, Z, middleEnergies)
    return middleEnergies, flux.T


@jit(nopython=True)
def getModifiedISO_GCR_Flux_Energy_Arr(W: float, Z: int, energy: np.ndarray) -> np.ndarray:
    if (np.logical_or(W < 0, W > 200)):
        raise Exception("parameter W  not valid; range is 0<=W<=200")
    elif ((Z < 1) or (Z > 28)):
        raise Exception("atomic number Z not valid; range is 1<=Z<=28")
    elif np.any(energy < 10.0):
        ...
        c = CI[i]
        alpha = alphaI[i]
        # had to use gamm as the variable name here rather than gamma, as gamma is already used in Fortran
        gamm = gammaI[i]
        ...
        delta = 0.02*W+4.7
        phi = c*(beta**alpha)/(rigidity**gamm) * \
            ((rigidity/(rigidity+R0))**delta)

        outputFlux = 0.0001*phi*A/Z*0.001/beta
    return outputFlux


def getModifiedISO_GCR_Flux_Energy_Arr_Time_Series(W_arr, Z, energy):
    if np.any(np.logical_or(W_arr < 0, W_arr > 200)):
        raise Exception("parameter W  not valid; range is 0<=W<=200")
    elif ((Z < 1) or (Z > 28)):
        raise Exception("atomic number Z not valid; range is 1<=Z<=28")
    elif np.any(energy < 10.0):
        ...
        #       using the main modified ISO model equation to calculate the output flux
        # ==============================================================================
        R0 = (0.37+0.0003*(W_arr**1.45))
        mod_rig = rigidity + R0[:, np.newaxis]

        delta = 0.02*W_arr+4.7

        coefficient_ratio = c*(beta**alpha)/(rigidity**gamm)
        power_term = np.power(rigidity/mod_rig, delta[:, np.newaxis])
        phi = coefficient_ratio * power_term
...
def getAtomicMass(atomicNumber):
    ...
    if (atomicNumber < 0):
        ...
        # look up table for other elements.
        atomicMass = A[atomicNumber-1]
    else:
...
if __name__ == "__main__":
    testW = 111.3
    testZ = 1
    defaultOutputFluxes = getModifiedISO_GCR_Flux_Default_Energies(
        testW, testZ)
    print("Success!")
