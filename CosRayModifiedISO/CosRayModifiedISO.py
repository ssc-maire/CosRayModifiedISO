...
def getSpectrumUsingSolarModulation(solarModulationWparameter: float, atomicNumber: int):
    generalSpectrum = ISOmodelSpectrum_fromSolarModulation(
        solarModulationWparameter, atomicNumber)
...
def getSpectrumUsingOULUcountRate(OULUcountRatePerSecond: float, atomicNumber: int):
    solarModulationWparameter = getWparameterFromOULUcountRate(
        OULUcountRatePerSecond)
    outputDF = getSpectrumUsingSolarModulation(
        solarModulationWparameter, atomicNumber)
    return outputDF
...
def getSpectrumUsingTimestamp(timestamp: dt.datetime, atomicNumber: int):
    ...
    if timestamp.tzinfo is None:
        logging.warning(
            "The inputted timestamp does not have timezone info. Assuming UTC.")
...
if __name__ == "__main__":
    ...
    print(getSpectrumUsingTimestamp(dt.datetime(
        year=1989, month=10, day=27), atomicNumber=1))
    print(getSpectrumUsingTimestamp(dt.datetime(
        year=2000, month=10, day=27), atomicNumber=1))
    print(getSpectrumUsingTimestamp(dt.datetime(
        year=2000, month=10, day=27), atomicNumber=7))
