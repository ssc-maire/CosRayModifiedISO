...
A simple library for running the ISO cosmic ray spectrum model as modified by DLR. All the details and equations about this model can be found in Matthiä et al., 
...
```
from DLRISOmodel import DLRISOmodel
```
...
(or range of kinetic energies), the appropriate method for `DLRISOmodel` is
```
DLRISOmodel.getEnergyFluxesFromEnergies(solarModulationWparameter, atomicNumber, energyListInMeV)
```
...
print(DLRISOmodel.getEnergyFluxesFromEnergies(solarModulationWparameter, atomicNumber, energyListInMeV))
```
gives
```
[0.00012419]
...
```
DLRISOmodel.getRigidityFluxesFromRigidities(solarModulationWparameter, atomicNumber, rigidityListInGV)
```
...
The method
```
DLRISOmodel.getSpectrumUsingTimestamp(timestamp, atomicNumber)
```
...
print(DLRISOmodel.getSpectrumUsingTimestamp(datetimeToUse,atomicNumber=1))
```
returns the output
```
...
```
DLRISOmodel.getSpectrumUsingOULUcountRate(OULUcountRatePerSecond, atomicNumber)
```
...
```
DLRISOmodel.getWparameterFromOULUcountRate(OULUcountRateInSeconds)
```
...
The methods
```
DLRISOmodel.convertParticleRigidityToEnergy(particleRigidityInGV, 
                                            particleMassAU, 
                                            particleChargeAU)
```
and
...
The methods
```
DLRISOmodel.convertParticleRigiditySpecToEnergySpec(particleRigidityInGV, 
...
and
```
DLRISOmodel.convertParticleEnergySpecToRigiditySpec(particleKineticEnergyInMeV, 
                                                    fluxInEnergyMeVform, 
                                                    particleMassAU, 
                                                    particleChargeAU)
```
...
```
DLRISOmodel.getAtomicMass(atomicNumber)
```
