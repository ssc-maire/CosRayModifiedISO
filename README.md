...
* Spectrum **energies** are kinetic energy **per nucleon** (**MeV/n**)
* Spectrum **rigidities** are **total** rigidity \(pc/Ze\) (**GV**), not GV/n
* Differential energy fluxes are **cts / cm<sup>2</sup> / s / sr / (MeV/n)**
* Differential rigidity fluxes are **cts / cm<sup>2</sup> / s / sr / GV**
* The low-level converters `convertParticleEnergyToRigidity` / `convertParticleRigidityToEnergy` take **total** kinetic energy in **MeV** and **total** rigidity in **GV**. To convert from MeV/n, multiply kinetic energy by mass number \(A\) first (or use `convertPerNucleonEnergyToTotalRigidity`).
...
or a range of differential fluxes corresponding to each supplied kinetic energy **per nucleon** (MeV/n). For protons this is the same as total MeV.
...
```
which has exactly the same syntax and usage as the `getEnergyFluxesFromEnergies` method, but where **total** rigidities in GV must be supplied instead 
of kinetic energies per nucleon.
...
returns the output
```python
    Energy (MeV/n)  d_Flux / d_E (cm-2 s-1 sr-1 (MeV/n)-1)  Rigidity (GV)  d_Flux / d_R (cm-2 s-1 sr-1 GV-1)
...
For \(Z>1\), the rigidity columns remain **total** GV (and flux per GV), not GV/n. Energy remains MeV/n.
...
These converters use **total** kinetic energy (MeV) and **total** rigidity (GV). They do not interpret energy as MeV/n.
